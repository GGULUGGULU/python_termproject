...
def update_parking_areas(event):
    ...
    if react1.status_code==200 and selected_airport!='ICN-인천국제공항':
        ...
        for i in range(len(parking_area_name_combo1)):
            ...
            canvas.create_text(x1 + bar_width / 2, y0 + 100, text=parking_area_name_combo1[i], anchor='n', angle=90)
            canvas.create_text(x1 + bar_width / 2, y1 - 10, text=parking_area_remaining_combo1[i], anchor='s')
